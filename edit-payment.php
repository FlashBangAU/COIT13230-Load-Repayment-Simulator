            ...
            if (isset($_POST['submit'])) {
                ...
                if (empty($_POST['date']) || empty($_POST['payment'])){
                    ...
                    if ($newDate === $date && $newPayment === $payment) {
                        ...
                        $query = "UPDATE additional_payments SET date_additional_payment = ?, amount_additional_payments = ? WHERE DB_set = ? AND payment_ID = ?";
                        $stmt = $db->prepare($query);

                        if ($stmt) {
                            $stmt->bind_param("sdii", $newDate, $newPayment, $DbID, $paymentID);
                            $stmt->execute();
            ...
            <form action="" method="POST">
                <table>
                    ...
                    <tr>
                        ...
                        <td><input type="text" name="payment" value="$payment" maxlength="20" required>
                    </tr>
                </table>
                <br>
